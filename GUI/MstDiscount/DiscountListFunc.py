...
from DiscountList import Ui_Form
import pyodbc
from DiscountDetailFunc import DiscountDetailFunc
from tkinter import *
...
class UserListFunc(QMainWindow):
    def __init__(self):
        ...
        # hide column id
        self.ui_form.tblDiscount.setColumnHidden(2, True)
    ...
    def load_data(self, page=1, custom_offset=None):
        ...
        if conn and cursor:
            try:
                ...
                query = f'''
                SELECT Id, DiscountCode, Discount, DiscountRate, IsLocked FROM MstDiscount ORDER BY Id ASC OFFSET {offset} ROWS FETCH NEXT {self.items_per_page} ROWS ONLY
                '''
                ...
                self.ui_form.tblDiscount.setRowCount(len(rows))
                self.ui_form.tblDiscount.setColumnCount(7)  # Set the appropriate number of columns
                    ...
                    for col_num, cell_value in enumerate(row_data[:7]):  # Use only the first 10 columns
                        for col in col_array:
                            if col == 0:
                                ...
                                self.ui_form.tblDiscount.setCellWidget(row_num, col, widget)
                        ...
                        self.ui_form.tblDiscount.setRowHeight(row_num,50)
                        self.ui_form.tblDiscount.setItem(row_num, col_num + 2, item)
                ...
                for row_num, row_data in enumerate(rows):
                    for col_num in range(6, 7):
                        ...
                        self.ui_form.tblDiscount.setItem(row_num, col_num, item)

                # Set each column width to fit the longest values from each row
                for col_num in range(7):
                    self.ui_form.tblDiscount.resizeColumnToContents(col_num)
    ...
    def search_data(self):
        ...
        if conn and cursor:
            try:
                ...
                query = f'''
                SELECT Id, DiscountCode, Discount, DiscountRate, IsLocked FROM MstDiscount WHERE Discount LIKE ?
                '''
                ...
                # Set the number of rows and columns for the QTableWidget
                self.ui_form.tblDiscount.setRowCount(len(rows))
                self.ui_form.tblDiscount.setColumnCount(6)  # Set the appropriate number of columns
    ...
    def btnAdd_clicked(self):
        self.destroy()
        self.ui_form = DiscountDetailFunc(0)
        self.ui_form.show()
        
        
    def btnEdit_clicked(self, cell_value):
        self.destroy()
        self.ui_form = DiscountDetailFunc(cell_value)
        self.ui_form.show()
        ...
        if answer == "yes":
            ...
            if conn and cursor:
                try:
                    # Modify the SQL query based on filter options
                    query = f'''
                    DELETE FROM MstDiscount WHERE Id = {cell_value}
                    '''
                    cursor.execute(query)
                    conn.commit()
