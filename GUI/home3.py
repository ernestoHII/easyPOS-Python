...
# from TrnPOS.TrnPOSTouchQuickService.POSTouchQuickServiceList import Ui_Form_POSList
# from ItemList import Ui_Form
...
# class POSTouchQuickServiceList(QWidget):
    # def __init__(self, main_window):
    #     super().__init__()
    #     self.main_window = main_window
    #     self.ui = Ui_Form_POSList()
    #     self.ui.setupUi(self)
...
class Menu(QMainWindow):
    ...
    def create_tab(self, button_value):
        if self.tab_widget is not None:
            ...
            # elif button_value == "POS - F2":  # This is the modification to handle POS - F2 button click
            #     self.add_POS_tab()
                # close_button.clicked.connect(self.close_current_tab)
                # add_button.clicked.connect(self.add_item_detail_tab)  # Connect the add_button click event to a new function

        # Lastly, set the current tab to the newly created tab.
        # index = self.tab_widget.indexOf(new_tab)
        # if index != -1:
        #     self.tab_widget.setCurrentIndex(index)
    ...
    def add_item_list_tab(self):
        item_list_tab = EmbeddedItemList(self)
        self.tab_widget.addTab(item_list_tab, "Setup - Item List")
        
        # Set the focus to the newly created "EmbeddedItemList" tab
        self.tab_widget.setCurrentWidget(item_list_tab)
        
    # def add_POS_tab(self):
    #     item_POS_tab = POSTouchQuickServiceList(self)
    #     self.tab_widget.addTab(item_POS_tab, "POS - F2")
        
    #     # Set the focus to the newly created "POSTouchQuickServiceList" tab
