...
from BankDetail.BankDetail import Ui_Form
import pyodbc
...
load_dotenv()
class BankDetailCombinedApp(QMainWindow):
    ...
    def insert_new(self):
        ...
        txtBank = self.ui_form.txtBank.toPlainText()
    ...
    def load_detail(self):
        ...
        if conn and cursor:
            try:
                ...
                query = f'''
                SELECT * FROM MstBank WHERE Id = {self.id}
                '''
                cursor.execute(query)
                ...
                for row in rows:
                    self.ui_form.txtBank.setText(row[1])
...
if __name__ == "__main__":
    ...
    window = BankDetailCombinedApp (0)
    window.show()
    sys.exit(app.exec_())