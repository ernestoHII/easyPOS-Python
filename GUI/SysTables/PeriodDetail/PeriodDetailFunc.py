...
from PeriodDetail.PeriodDetail import Ui_Form
import pyodbc
...
load_dotenv()
class PeriodDetailCombinedApp(QMainWindow):
    ...
    def insert_new(self):
        ...
        txtPeriod = self.ui_form.txtPeriod.toPlainText()
    ...
    def load_detail(self):
        ...
        if conn and cursor:
            try:
                ...
                query = f'''
                SELECT * FROM MstPeriod WHERE Id = {self.id}
                '''
                cursor.execute(query)
                ...
                for row in rows:
                    self.ui_form.txtPeriod.setText(row[1])
...
if __name__ == "__main__":
    ...
    window = PeriodDetailCombinedApp(0)
    window.show()
    sys.exit(app.exec_())