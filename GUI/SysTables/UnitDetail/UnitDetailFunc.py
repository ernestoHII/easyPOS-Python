...
from UnitDetail.UnitDetail import Ui_Form
import pyodbc
...
load_dotenv()
class UnitDetailCombinedApp(QMainWindow):
    ...
    def insert_new(self):
        ...
        txtUnit = self.ui_form.txtUnit.toPlainText()
    ...
    def load_detail(self):
        ...
        if conn and cursor:
            try:
                ...
                query = f'''
                SELECT * FROM MstUnit WHERE Id = {self.id}
                '''
                cursor.execute(query)
                ...
                for row in rows:
                    self.ui_form.txtUnit.setText(row[1])
...
if __name__ == "__main__":
    ...
    window = UnitDetailCombinedApp(0)
    window.show()
    sys.exit(app.exec_())