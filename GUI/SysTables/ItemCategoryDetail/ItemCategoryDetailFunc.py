...
from ItemCategoryDetail.ItemCategoryDetail import Ui_Form
import pyodbc
...
load_dotenv()
class ItemCategoryDetailCombinedApp(QMainWindow):
    ...
    def insert_new(self):
        ...
        txtCategory = self.ui_form.txtCategory.toPlainText()
    ...
    def load_detail(self):
        ...
        if conn and cursor:
            try:
                ...
                query = f'''
                SELECT * FROM MstItemCategory WHERE Id = {self.id}
                '''
                cursor.execute(query)
                ...
                for row in rows:
                    self.ui_form.txtCategory.setText(row[1])
...
if __name__ == "__main__":
    ...
    window = ItemCategoryDetailCombinedApp (0)
    window.show()
    sys.exit(app.exec_())